...
| Non-destructive layers | Supported via `Terrain3DCurveLayerPath`, which bakes absolute heights from a Curve3D (optionally offset via `depth`) so you can stamp rivers, roads, or rails without destructive sculpting. Use the optional `falloff_curve` to shape the cross-section profile (0 = edge, 1 = center).

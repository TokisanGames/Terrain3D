...
No matter how good of assets you have, your environment will not look good without a proper rendering environment. 
...
4. A good foliage shader (below). The default one is inadequate.
5. Suitable Environment settings to balance and polish your look.
...
### Room Lighting & Monitor Calibration
...
* Your monitor is slightly blue, which matches outdoor light. Incandescent lights are orange. Cool white bulbs are too blue. You want the room lighting as close to neutral as possible, which is shaded or cloudy outdoor lighting.
* Before buying a screen or laptop:
	...
	* Look at it in person. Open a web browser and load up an image calibration image. Try to adjust the screen in the store to ensure you can get accurate results at home.
	* Look up the monitor on a review website that focuses on screen testing.
	* Lenovo laptop monitors have been very good in recent years (through 2024 at least).
* Buy a device to calibrate your monitor like something from Calibrite or Datacolor. Or:
* Manually calibrate your monitor:
...
You should familiarize yourself with the Godot docs on [WorldEnvironment](https://docs.godotengine.org/en/stable/classes/class_worldenvironment.html), [Environment](https://docs.godotengine.org/en/stable/classes/class_environment.html), [Camera3D](https://docs.godotengine.org/en/stable/classes/class_camera3d.html), and [CameraAttributes](https://docs.godotengine.org/en/stable/classes/class_cameraattributes.html) and its derivatives *Practical and *Physical, and [DirectionalLight3D](https://docs.godotengine.org/en/stable/classes/class_directionallight3d.html). There are many options that need to be tweaked for a good looking setup, which you can learn over time.
...
* [AllSkyFree](https://github.com/rpgwhitelock/AllSkyFree_Godot) - 8-bit PNGs, but they are attractive, game-centric and free; with paid options. 8-bit may be fine for some projects.
* [Polyhaven](https://polyhaven.com/hdris)
* [HDRI-Skies](https://hdri-skies.com/free-hdris/)
* [HDRMaps](https://hdrmaps.com/freebies/)

Once you have the file, resize it according to how much VRAM you want to consume, and how high of quality you want. 2k-4k is most likely. Don't use it at 16k.
...
You can write your own sky material or look online for already written sky shaders. Place a `ShaderMaterial` in `WorldEnvironment/Environment/Sky`. For a shader developer there's no limit to what you can do. You could have clouds, stars, multiple suns, multiple moons. All of this can provide reflective light that you'll see as psuedo bounce lighting and in reflections.
...
The default material in Godot is sub par for foliage. Here are some tips to improve rendering. However **[setup your lighting](#lighting) first!** You cannot properly adjust your materials if you aren't seeing objective "reality".
...
With 3D foliage, the leaves are 2D textures UV mapped onto flat polygons cards distributed around 3D polygon branches.
...
You can achieve a similar affect in the shader by skipping the automatic vertex transform, calculating the vertex transform yourself, and calculating vertex normals as they extend outward from the origin of the tree.
...
void vertex() {
	...
	NORMAL = normalize(VERTEX); // Re calculate vertex normals from origin
...
Normals applied by textures are done so in `NORMAL_MAP`, whereas normals applied by the vertices and faces are done in `NORMAL`.

Sometimes the normal map you got from an asset pack are not OpenGL or DirectX as you expected. Unlike with rocks, it's often much harder to tell which is correct for foliage. 

You can convert between OpenGL and DirectX by inverting the green channel either in Photoshop, in the Godot import settings, or with the shader code below. The latter is useful to be able to quickly compare how the light reflects off of the leaves. Make sure you're looking at the front face of the texture card (see above).
...
* Material textures don't have to be the same resolution. You could have an 8k albedo texture that covers a lot of plants, and find that a 2k or 4k normal map is sufficient. Zoom your camera as close to the plant leaves as your player will be. If you can't tell a difference between a 2k, 4k or 8k normal map or roughness texture, then you don't need the latter.


### Thinning Trees

The Godot renderer will quickly discard pixels of thin foliage as the camera moves away. To combat this, prefer broad leaf foliage over thin leaves. These snippets will also help.

* This code makes trees maintain volume at a distance. You can change `log` to `log2` or even remove it for more thickness.
...
* Some foliage renders with a white edge even though it's not visible on the texture. Check if it's an issue with roughness or specular and adjust as above. However if the issue is in ALBEDO, this line placed *after* assigning the texture to ALPHA will allow you to clip off white pixels.
```glsl

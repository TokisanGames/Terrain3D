...
The control maps are queried for each of the 4 grid points and stored in `control[0]`-`control[3]`. The control map bit packed format is defined in [Controlmap Format](controlmap_format.md).

The control maps are then parsed into an array of control_data structs, which contain the `texture_id[]`, `texture_weight[]`, the `blend` value used in determinning the texture weight values, and a the decoded scale and angle values.

If the Autoshader is enabled, the control_data for each of the 4 gridpoints is updated with the autoshader ids and the autoshader blend value calculated from the terrain normal, and height.

Control_data is then iterated over to calculate the `texture_weight[]` of each `texture_id[]`. This takes into account how many of a given texture are present, and the bilinear weight. Eg if an ID is present at only 3 of 4 index points, then its weight would be:

`weights[0] * blend_weight[0] + weights[2] * blend_weight[2] + weights[3] * blend_weight[3]`

which allows smooth interpolation, and better blend shaping, across indicies during the material accumulation.

The textures at each vertex are looked up and accumulated, the weight for each lookup is modified by its height value, sharpness, and potentially the world normal of any the ID in a lower slot in the same index.

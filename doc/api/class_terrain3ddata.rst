...
.. table::
   ...
   +----------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Error                                                                      | :ref:`export_image<class_Terrain3DData_method_export_image>`\ (\ file_name\: ``String``, map_type\: :ref:`MapType<enum_Terrain3DRegion_MapType>` = 0, mode\: :ref:`ExportMode<enum_Terrain3DData_ExportMode>` = 0\ ) |const| |
   +----------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   +----------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``Image``                                                                  | :ref:`layered_to_image<class_Terrain3DData_method_layered_to_image>`\ (\ map_type\: :ref:`MapType<enum_Terrain3DRegion_MapType>`, bounds\: ``Rect2i`` = Rect2i(0, 0, 0, 0)\ ) |const|                                        |
   +----------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
.. _enum_Terrain3DData_ExportMode:

.. rst-class:: classref-enumeration

enum **ExportMode**: :ref:`🔗<enum_Terrain3DData_ExportMode>`

.. _class_Terrain3DData_constant_EXPORT_SLICES:

.. rst-class:: classref-enumeration-constant

:ref:`ExportMode<enum_Terrain3DData_ExportMode>` **EXPORT_SLICES** = ``0``

The terrain is exported in up to 16k slices, as many as are needed.

.. _class_Terrain3DData_constant_EXPORT_REGIONS:

.. rst-class:: classref-enumeration-constant

:ref:`ExportMode<enum_Terrain3DData_ExportMode>` **EXPORT_REGIONS** = ``1``

The terrain is exported as one Image per region. Filenames receive the region coordinates as a suffix, e.g. -01_03.exr
...
Error **export_image**\ (\ file_name\: ``String``, map_type\: :ref:`MapType<enum_Terrain3DRegion_MapType>` = 0, mode\: :ref:`ExportMode<enum_Terrain3DData_ExportMode>` = 0\ ) |const| :ref:`🔗<class_Terrain3DData_method_export_image>`
...
R16 can be edited by Krita, however you must know the dimensions and min/max before reimporting. This information is printed to the console by this function. The global height range is used for r16 if previously calculated.

Res/tres stores in Godot's native data format.

Mode specifies exporting up to 16k x 16k slices or one file per region.
...
Returns the associated pixel on the color map at the requested position.  See :ref:`get_pixel()<class_Terrain3DData_method_get_pixel>`.
...
Note: If using :ref:`Terrain3D.color_compress_mode<class_Terrain3D_property_color_compress_mode>`, Godot ``Image.set_pixel()`` and ``get_pixel()`` do not work on compressed images and will print errors at runtime.
...
Returns the roughness modifier (wetness) on the color map alpha channel associated with the specified position. See :ref:`get_pixel()<class_Terrain3DData_method_get_pixel>`.
...
``Image`` **layered_to_image**\ (\ map_type\: :ref:`MapType<enum_Terrain3DRegion_MapType>`, bounds\: ``Rect2i`` = Rect2i(0, 0, 0, 0)\ ) |const| :ref:`🔗<class_Terrain3DData_method_layered_to_image>`

Returns an Image of the given map type that contains all regions in one large image.

Bounds allows restricting the total world space, as the largest supported Image size is 16,384 x 16,384. Disregard vertex_spacing and use descaled bounds.

If the world has multiple data islands with empty regions in between, this function will return an Image large enough to encompass all of the used regions, with blank areas in between the regions.

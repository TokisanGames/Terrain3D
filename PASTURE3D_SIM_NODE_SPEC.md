...
| [bench/PreviewSimDiag.tscn](project/bench/PreviewSimDiag.gd) | §21.8's re-test of the preview complaint. **A diagnosis, not a gate** — it asserts nothing and prints the three divergences it measured. **All three are now fixed**, and gates BJ–BL hold them fixed — the diagnosis is kept because it is what a re-test looks like before there is anything to assert | 6.5 |
...
| [bench/SimPhase7Gate.tscn](project/bench/SimPhase7Gate.gd) | Gates AO–AR, **all four headless** — the threaded solve is bitwise the synchronous one, Cancel joins, teardown is safe, and the worst main-thread frame falls from 140.8 ms to 10.0 ms. §20.7 predicted AO and AR would need an editor and was half wrong; what genuinely does is narrower, and the gate prints it as a checklist | 7 |
...
| **7 — BUILT (§20), AO–AR passing** | The pure half of the solve moves onto a worker thread. The profiling §20.6 demanded was run 2026-08-19: **the commit is 0.4 % of a full-resolution build**, so 99.6 % of it is what phase 7 would move and the phase is worth building. The same pass found **depression filling is 61 % of the solve** (§11), which made the cheaper win the better thing to do first — and it was done first: the O(n) monotone bucket queue replaced the binary heap before this phase landed, for 1.13× on the flood, gated bitwise by **BI**. The flood is memory-bound, not comparison-bound, which is why 1.13× is all an O(n) queue buys |
| **8 — SPECCED 2026-08-20, not built** | Let a landform brush's relief selectors read its OWN generated profile. Today a Mound's selector reads the ground *under* the Mound, so on flat ground every filter type returns one constant and "craggy on the flanks, smooth on top" cannot be expressed. Surfaced by the §18 preview; see §15.10. Specced as **phase 1 of [PASTURE3D_BRUSH_EROSION_SPEC.md](PASTURE3D_BRUSH_EROSION_SPEC.md)**, gates BM–BQ — widened there, because the same missing field also fixes `TERRACE`/`STRATIFY` banding a fractal instead of the hill. That spec also carries phases for sediment transport (§15.1, §15.8 — which **invalidates gate R**), brush-hosted erosion, and multi-resolution amplification |

...
func matches_search(value: Terrain3DAssetResource) -> bool:
	return value.name.containsn(search_text)
...
func update_list_entries() -> void:
	...
	if not search_text.is_empty():
		display_list = display_list.filter(matches_search)
...
func _on_edit_clicked(p_entry: Terrain3DListEntry) -> void:
	selected_list = [ p_entry.resource.id ]
	update_selection()
...
func _on_highlight_clicked(p_entry: Terrain3DListEntry) -> void:
	...
	if p_entry.resource.id in selected_list:
		...
		for entry: Terrain3DListEntry in _entries:
			if not entry.resource:
				continue
			if entry.resource.id in selected_list:
...
func _on_clear_clicked(p_entry: Terrain3DListEntry) -> void:
	...
	if p_entry.resource.id in selected_list:
		clear_resources(selected_list)
	else:
		clear_resources([p_entry.resource.id])
...
func clear_resources(p_asset_ids: Array[int]) -> void:
	var asset_dock: Control = get_parent().get_parent().get_parent()
	if type == Terrain3DAssets.TYPE_TEXTURE:
		asset_dock.confirm_dialog.dialog_text = "Are you sure you want to clear %s texture%s?" % [p_asset_ids.size(), "" if p_asset_ids.is_empty() else "s"]
	else:
		asset_dock.confirm_dialog.dialog_text = "Are you sure you want to clear %s mesh%s and delete all instances?" % [p_asset_ids.size(), "" if p_asset_ids.is_empty() else "es"]
	asset_dock.confirm_dialog.popup_centered()
	await asset_dock.confirmation_closed
	if not asset_dock._confirmed:
		return
	selected_list = [ 0 ]
	for asset_id: int in p_asset_ids:
		update_resource(null, asset_id)
	for key: int in _entry_idx_to_asset_id_map.keys():
		if _entry_idx_to_asset_id_map[key] in p_asset_ids:
			if key == _entries.size() - 1:
				_entries[key].set_edited_resource(null, false)
				continue
			if type == Terrain3DAssets.TYPE_TEXTURE:
				_entries[key].set_edited_resource(Terrain3DTextureAsset.new(), false)
			else:
				_entries[key].set_edited_resource(Terrain3DMeshAsset.new(), false)
	EditorInterface.inspect_object(null)
	update_asset_list()


func update_resource(p_resource: Resource, p_id: int) -> void:

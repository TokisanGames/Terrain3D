...
func eval_grid_channels(p_inputs: Array, p_gw: int, p_gh: int, _p_mask, p_rect: Rect2) -> Array:
	...
	if ClassDB.class_has_method("Pasture3DUtil", "mountain_inselberg_generate_grid_best"):
		res = Pasture3DUtil.mountain_inselberg_generate_grid_best(p_gw, p_gh, p_rect, params)
	else:
		res = Pasture3DUtil.mountain_inselberg_generate_grid(p_gw, p_gh, p_rect, params)
	return [res]

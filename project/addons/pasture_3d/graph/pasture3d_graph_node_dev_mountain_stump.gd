@tool
class_name Pasture3DGraphNodeDevMountainStump
extends Pasture3DGraphNode

## Development reference oracle for MountainStump.
...
func op() -> StringName:
	return &"dev_mountain_stump"


func display_name() -> String:
	return "[Dev/GD] Mountain Stump"
...
static func _minimum_smooth(a: float, b: float, k: float) -> float:
	var h: float = clampf(0.5 + 0.5 * (b - a) / maxf(1e-5, k), 0.0, 1.0)
...
static func solve_oracle(p_gw: int, p_gh: int, _p_rect: Rect2, p_params: Dictionary) -> PackedFloat32Array:
	...
	var k_smoothing_val: float = maxf(0.001, float(p_params.get("k_smoothing", 0.05)))
	...
	var half_width: float = 0.1 * scale_val
	...
	for iz in range(p_gh):
		...
		for ix in range(p_gw):
			...
			var n_disp: float = scale_val * base_noise_amp_val * _simplex2_fbm(nx * kw, ny * kw, octaves_val, persistence, lacunarity, seed_u)
			var disp_x: float = n_disp * cos(alpha) + (dx[idx] if has_dx else 0.0)
			var disp_y: float = n_disp * sin(alpha) + (dy[idx] if has_dy else 0.0)
			...
			var pulse: float = minf(1.0, 2.0 * exp(-0.5 * r2))

			var stump_n: float = 0.25 * _simplex2_fbm((nx + disp_x) * kw, (ny + disp_y) * kw, octaves_val, persistence, lacunarity, seed_u + 71) + 0.75
			var stump: float = _minimum_smooth(stump_n, pulse, k_smoothing_val)

			var vx: float = (nx + disp_x) * kw
			var vy: float = (ny + disp_y) * kw
			var vor: float = 2.0 * _voronoi_edge_fbm(vx, vy, octaves_val, persistence, lacunarity, seed_u + 149)
			vor = minf(1.0, vor) * pulse

			var val: float = stump + ridge_amp_val * vor
			if gamma_val > 0.0:
				val = pow(maxf(0.0, val), gamma_val)

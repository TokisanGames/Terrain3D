...
func _f_metadata_and_warnings() -> void:
	...
	var want := PackedStringArray(["amplitude", "ridge_offset", "erosion_accent", "gain", "frequency"])
	if s.input_count() != want.size():
		_fail += 1; print("    !! input_count() = %d, want %d" % [s.input_count(), want.size()])
	elif s.input_names() != want:
		_fail += 1; print("    !! input_names() = %s, want %s" % [s.input_names(), want])
	else:
		for k in want.size():
			if s.input_port_types()[k] != Pasture3DGraphNode.PortType.FLOAT:

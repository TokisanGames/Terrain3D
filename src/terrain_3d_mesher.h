...
public:
	...
	void set_material(const RID &p_material) { SET_IF_DIFF(_material, p_material); }
	RID get_material() const { return _material; }
	void set_tessellation_level(const int p_level) { SET_IF_DIFF(_tessellation_level, p_level); }
	int get_tessellation_level() const { return _tessellation_level; }
	void set_mesh_size(const int p_size) { SET_IF_DIFF(_mesh_size, p_size); }

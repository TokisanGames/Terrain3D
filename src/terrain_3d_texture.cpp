...
void Terrain3DTexture::set_uv_detiling(real_t p_rotation) {
	_data._uv_detiling = CLAMP(p_rotation, 0.0f, 1.0f);
	emit_signal("setting_changed");
}
...
void Terrain3DTexture::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_uv_detiling", "scale"), &Terrain3DTexture::set_uv_detiling);
	ClassDB::bind_method(D_METHOD("get_uv_detiling"), &Terrain3DTexture::get_uv_detiling);
	...
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "uv_detiling", PROPERTY_HINT_RANGE, "0.0, 1.0"), "set_uv_detiling", "get_uv_detiling");
}

...
void Pasture3DMaterial::_preload_shaders() {
 ...
 */
void Pasture3DMaterial::_parse_shader(const String &p_shader, const String &p_name) {
	if (p_name.is_empty()) {
 ...
 */
String Pasture3DMaterial::_apply_inserts(const String &p_shader, const Array &p_excludes) const {
	PackedStringArray parsed = p_shader.split("//INSERT:");
	String shader;
	for (int i = 0; i < parsed.size(); i++) {
		// First section of the file before any //INSERT:
...
String Pasture3DMaterial::_strip_comments(const String &p_shader) const {
...
String Pasture3DMaterial::_inject_editor_code(const String &p_shader) const {
	...
	// Editor Functions
	if (_show_navigation || (_terrain && _terrain->get_editor() && _terrain->get_editor()->get_tool() == Pasture3DEditor::NAVIGATION)) {
		insert_names.push_back("EDITOR_NAVIGATION");
	}
	if (_show_region_grid || (_terrain && _terrain->get_editor() && _terrain->get_editor()->get_tool() == Pasture3DEditor::REGION)) {
...
void Pasture3DMaterial::_update_shader() {
	IS_INIT(VOID);
	...
	if (_shader_override_enabled && _shader_override.is_valid()) {
		...
		if (!_shader_override->is_connected("changed", callable_mp(this, &Pasture3DMaterial::_update_shader))) {
			LOG(DEBUG, "Connecting changed signal to _update_shader()");
			_shader_override->connect("changed", callable_mp(this, &Pasture3DMaterial::_update_shader));
		}
	} else {
	...
	if (_terrain->get_tessellation_level() > 0) {
		if (_buffer_shader_override_enabled && _buffer_shader_override.is_valid()) {
			...
			if (!_buffer_shader_override->is_connected("changed", callable_mp(this, &Pasture3DMaterial::_update_shader))) {
				LOG(DEBUG, "Connecting changed signal to _update_shader()");
				_buffer_shader_override->connect("changed", callable_mp(this, &Pasture3DMaterial::_update_shader));
			}
		} else {
...
void Pasture3DMaterial::_update_uniforms(const RID &p_material, const uint32_t p_flags) {
	...
	Ref<Pasture3DAssets> asset_list = _terrain->get_assets();
...
void Pasture3DMaterial::_set_shader_parameters(const Dictionary &p_dict) {
...
// This function serves as the constructor which is initialized by the class Pasture3D.
// Godot likes to create resource objects at startup, so this prevents it from creating
// uninitialized materials.
void Pasture3DMaterial::initialize(Pasture3D *p_terrain) {
	if (p_terrain) {
		_terrain = p_terrain;
	} else {
...
void Pasture3DMaterial::destroy() {
...
void Pasture3DMaterial::update(uint32_t p_flags) {
...
void Pasture3DMaterial::set_displacement_scale(const real_t p_displacement_scale) {
...
void Pasture3DMaterial::set_displacement_sharpness(const real_t p_displacement_sharpness) {
...
void Pasture3DMaterial::set_world_background(const WorldBackground p_background) {
	SET_IF_DIFF(_world_background, p_background);
	LOG(INFO, "Enable world background: ", p_background);
	_update_shader();
}

void Pasture3DMaterial::set_texture_filtering(const TextureFiltering p_filtering) {
	SET_IF_DIFF(_texture_filtering, p_filtering);
	LOG(INFO, "Setting texture filtering: ", p_filtering);
	_update_shader();
}

void Pasture3DMaterial::set_auto_shader_enabled(const bool p_enabled) {
	SET_IF_DIFF(_auto_shader_enabled, p_enabled);
	LOG(INFO, "Enable auto shader: ", p_enabled);
	_update_shader();
}

void Pasture3DMaterial::set_dual_scaling_enabled(const bool p_enabled) {
	SET_IF_DIFF(_dual_scaling_enabled, p_enabled);
	LOG(INFO, "Enable dual scaling: ", p_enabled);
	_update_shader();
}

void Pasture3DMaterial::set_macro_variation_enabled(const bool p_enabled) {
	SET_IF_DIFF(_macro_variation_enabled, p_enabled);
	LOG(INFO, "Enable macro variation: ", p_enabled);
	_update_shader();
}

void Pasture3DMaterial::set_projection_enabled(const bool p_enabled) {
...
void Pasture3DMaterial::set_shader_override(const Ref<Shader> &p_shader) {
	SET_IF_DIFF(_shader_override, p_shader);
	LOG(INFO, "Setting override shader");
	_update_shader();
}

void Pasture3DMaterial::set_buffer_shader_override_enabled(const bool p_enabled) {
...
void Pasture3DMaterial::set_buffer_shader_override(const Ref<Shader> &p_shader) {
...
Error Pasture3DMaterial::save(const String &p_path) {
...
bool Pasture3DMaterial::_property_can_revert(const StringName &p_name) const {
...
bool Pasture3DMaterial::_property_get_revert(const StringName &p_name, Variant &r_property) const {
...
bool Pasture3DMaterial::_set(const StringName &p_name, const Variant &p_property) {
...
bool Pasture3DMaterial::_get(const StringName &p_name, Variant &r_property) const {
...
void Pasture3DMaterial::_bind_methods() {
	...
	// Private
	ClassDB::bind_method(D_METHOD("_set_shader_parameters", "dict"), &Pasture3DMaterial::_set_shader_parameters);
	ClassDB::bind_method(D_METHOD("_get_shader_parameters"), &Pasture3DMaterial::_get_shader_parameters);
	...
	ClassDB::bind_method(D_METHOD("set_world_background", "background"), &Pasture3DMaterial::set_world_background);
	ClassDB::bind_method(D_METHOD("get_world_background"), &Pasture3DMaterial::get_world_background);
	ClassDB::bind_method(D_METHOD("set_texture_filtering", "filtering"), &Pasture3DMaterial::set_texture_filtering);
	ClassDB::bind_method(D_METHOD("get_texture_filtering"), &Pasture3DMaterial::get_texture_filtering);
	ClassDB::bind_method(D_METHOD("set_auto_shader_enabled", "enabled"), &Pasture3DMaterial::set_auto_shader_enabled);
	ClassDB::bind_method(D_METHOD("get_auto_shader_enabled"), &Pasture3DMaterial::get_auto_shader_enabled);
	ClassDB::bind_method(D_METHOD("set_dual_scaling_enabled", "enabled"), &Pasture3DMaterial::set_dual_scaling_enabled);
	ClassDB::bind_method(D_METHOD("get_dual_scaling_enabled"), &Pasture3DMaterial::get_dual_scaling_enabled);
	...
	ClassDB::bind_method(D_METHOD("is_shader_override_enabled"), &Pasture3DMaterial::is_shader_override_enabled);
	ClassDB::bind_method(D_METHOD("set_shader_override", "shader"), &Pasture3DMaterial::set_shader_override);
	ClassDB::bind_method(D_METHOD("get_shader_override"), &Pasture3DMaterial::get_shader_override);

	ClassDB::bind_method(D_METHOD("set_buffer_shader_override_enabled", "enabled"), &Pasture3DMaterial::set_buffer_shader_override_enabled);
	ClassDB::bind_method(D_METHOD("is_buffer_shader_override_enabled"), &Pasture3DMaterial::is_buffer_shader_override_enabled);
	ClassDB::bind_method(D_METHOD("set_buffer_shader_override", "shader"), &Pasture3DMaterial::set_buffer_shader_override);
	ClassDB::bind_method(D_METHOD("get_buffer_shader_override"), &Pasture3DMaterial::get_buffer_shader_override);
	ClassDB::bind_method(D_METHOD("set_displacement_scale", "scale"), &Pasture3DMaterial::set_displacement_scale);
	ClassDB::bind_method(D_METHOD("get_displacement_scale"), &Pasture3DMaterial::get_displacement_scale);
	ClassDB::bind_method(D_METHOD("set_displacement_sharpness", "sharpness"), &Pasture3DMaterial::set_displacement_sharpness);
	ClassDB::bind_method(D_METHOD("get_displacement_sharpness"), &Pasture3DMaterial::get_displacement_sharpness);

	ClassDB::bind_method(D_METHOD("set_shader_param", "name", "value"), &Pasture3DMaterial::set_shader_param);
	ClassDB::bind_method(D_METHOD("get_shader_param", "name"), &Pasture3DMaterial::get_shader_param);

	// PBR output
	ClassDB::bind_method(D_METHOD("set_output_albedo_enabled", "enabled"), &Pasture3DMaterial::set_output_albedo_enabled);
	ClassDB::bind_method(D_METHOD("get_output_albedo_enabled"), &Pasture3DMaterial::get_output_albedo_enabled);
	ClassDB::bind_method(D_METHOD("set_output_roughness_enabled", "enabled"), &Pasture3DMaterial::set_output_roughness_enabled);
	ClassDB::bind_method(D_METHOD("get_output_roughness_enabled"), &Pasture3DMaterial::get_output_roughness_enabled);
	ClassDB::bind_method(D_METHOD("set_output_specular_enabled", "enabled"), &Pasture3DMaterial::set_output_specular_enabled);
	ClassDB::bind_method(D_METHOD("get_output_specular_enabled"), &Pasture3DMaterial::get_output_specular_enabled);
	ClassDB::bind_method(D_METHOD("set_output_normal_map_enabled", "enabled"), &Pasture3DMaterial::set_output_normal_map_enabled);
	ClassDB::bind_method(D_METHOD("get_output_normal_map_enabled"), &Pasture3DMaterial::get_output_normal_map_enabled);
	ClassDB::bind_method(D_METHOD("set_output_ambient_occlusion_enabled", "enabled"), &Pasture3DMaterial::set_output_ambient_occlusion_enabled);
	ClassDB::bind_method(D_METHOD("get_output_ambient_occlusion_enabled"), &Pasture3DMaterial::get_output_ambient_occlusion_enabled);

	// Overlays
	ClassDB::bind_method(D_METHOD("set_show_region_grid", "enabled"), &Pasture3DMaterial::set_show_region_grid);
	ClassDB::bind_method(D_METHOD("get_show_region_grid"), &Pasture3DMaterial::get_show_region_grid);
	ClassDB::bind_method(D_METHOD("set_show_instancer_grid", "enabled"), &Pasture3DMaterial::set_show_instancer_grid);
	ClassDB::bind_method(D_METHOD("get_show_instancer_grid"), &Pasture3DMaterial::get_show_instancer_grid);
	ClassDB::bind_method(D_METHOD("set_show_vertex_grid", "enabled"), &Pasture3DMaterial::set_show_vertex_grid);
	ClassDB::bind_method(D_METHOD("get_show_vertex_grid"), &Pasture3DMaterial::get_show_vertex_grid);
	ClassDB::bind_method(D_METHOD("set_show_contours", "enabled"), &Pasture3DMaterial::set_show_contours);
	ClassDB::bind_method(D_METHOD("get_show_contours"), &Pasture3DMaterial::get_show_contours);
	ClassDB::bind_method(D_METHOD("set_show_navigation", "enabled"), &Pasture3DMaterial::set_show_navigation);
	ClassDB::bind_method(D_METHOD("get_show_navigation"), &Pasture3DMaterial::get_show_navigation);

	// Debug Views
	ClassDB::bind_method(D_METHOD("set_show_checkered", "enabled"), &Pasture3DMaterial::set_show_checkered);
	ClassDB::bind_method(D_METHOD("get_show_checkered"), &Pasture3DMaterial::get_show_checkered);
	ClassDB::bind_method(D_METHOD("set_show_grey", "enabled"), &Pasture3DMaterial::set_show_grey);
	ClassDB::bind_method(D_METHOD("get_show_grey"), &Pasture3DMaterial::get_show_grey);
	ClassDB::bind_method(D_METHOD("set_show_heightmap", "enabled"), &Pasture3DMaterial::set_show_heightmap);
	ClassDB::bind_method(D_METHOD("get_show_heightmap"), &Pasture3DMaterial::get_show_heightmap);
	ClassDB::bind_method(D_METHOD("set_show_jaggedness", "enabled"), &Pasture3DMaterial::set_show_jaggedness);
	ClassDB::bind_method(D_METHOD("get_show_jaggedness"), &Pasture3DMaterial::get_show_jaggedness);
	ClassDB::bind_method(D_METHOD("set_show_autoshader", "enabled"), &Pasture3DMaterial::set_show_autoshader);
	ClassDB::bind_method(D_METHOD("get_show_autoshader"), &Pasture3DMaterial::get_show_autoshader);
	ClassDB::bind_method(D_METHOD("set_show_control_texture", "enabled"), &Pasture3DMaterial::set_show_control_texture);
	ClassDB::bind_method(D_METHOD("get_show_control_texture"), &Pasture3DMaterial::get_show_control_texture);
	ClassDB::bind_method(D_METHOD("set_show_control_blend", "enabled"), &Pasture3DMaterial::set_show_control_blend);
	ClassDB::bind_method(D_METHOD("get_show_control_blend"), &Pasture3DMaterial::get_show_control_blend);
	ClassDB::bind_method(D_METHOD("set_show_control_angle", "enabled"), &Pasture3DMaterial::set_show_control_angle);
	ClassDB::bind_method(D_METHOD("get_show_control_angle"), &Pasture3DMaterial::get_show_control_angle);
	ClassDB::bind_method(D_METHOD("set_show_control_scale", "enabled"), &Pasture3DMaterial::set_show_control_scale);
	ClassDB::bind_method(D_METHOD("get_show_control_scale"), &Pasture3DMaterial::get_show_control_scale);
	ClassDB::bind_method(D_METHOD("set_show_colormap", "enabled"), &Pasture3DMaterial::set_show_colormap);
	ClassDB::bind_method(D_METHOD("get_show_colormap"), &Pasture3DMaterial::get_show_colormap);
	ClassDB::bind_method(D_METHOD("set_show_roughmap", "enabled"), &Pasture3DMaterial::set_show_roughmap);
	ClassDB::bind_method(D_METHOD("get_show_roughmap"), &Pasture3DMaterial::get_show_roughmap);

	// PBR Views
	ClassDB::bind_method(D_METHOD("set_show_texture_albedo", "enabled"), &Pasture3DMaterial::set_show_texture_albedo);
	ClassDB::bind_method(D_METHOD("get_show_texture_albedo"), &Pasture3DMaterial::get_show_texture_albedo);
	ClassDB::bind_method(D_METHOD("set_show_texture_height", "enabled"), &Pasture3DMaterial::set_show_texture_height);
	ClassDB::bind_method(D_METHOD("get_show_texture_height"), &Pasture3DMaterial::get_show_texture_height);
	ClassDB::bind_method(D_METHOD("set_show_texture_normal", "enabled"), &Pasture3DMaterial::set_show_texture_normal);
	ClassDB::bind_method(D_METHOD("get_show_texture_normal"), &Pasture3DMaterial::get_show_texture_normal);
	ClassDB::bind_method(D_METHOD("set_show_texture_ao", "enabled"), &Pasture3DMaterial::set_show_texture_ao);
	ClassDB::bind_method(D_METHOD("get_show_texture_ao"), &Pasture3DMaterial::get_show_texture_ao);
	ClassDB::bind_method(D_METHOD("set_show_texture_rough", "enabled"), &Pasture3DMaterial::set_show_texture_rough);
	...
	// Hidden in Material, aliased in Pasture3D

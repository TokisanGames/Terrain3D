	...
	for (int r = 0; r < region_locations.size(); r++) {
		...
		for (int m = 0; m < mesh_types.size(); m++) {
			...
			for (int c = 0; c < cell_locations.size(); c++) {
				...
				for (int lod = Terrain3DMeshAsset::MAX_LOD_COUNT - 1; lod >= Terrain3DMeshAsset::SHADOW_LOD_ID; lod--) {
					...
					// reuse existing MM if we're updating the shadow lod
					mmi = cell_mmi_dict[cell];
					Ref<MultiMesh> mm;
					...
					if (lod == ma->get_first_shadow_lod()) {
...
Ref<MultiMesh> Terrain3DInstancer::_create_multimesh(const int p_mesh_id, const int p_lod, const TypedArray<Transform3D> &p_xforms, const PackedColorArray &p_colors) const {
	...
	Ref<Mesh> mesh = mesh_asset->get_lod_mesh(p_lod);
	if (mesh.is_null()) {
		LOG(ERROR, "No lod for mesh id ", p_mesh_id, " lod ", p_lod, " found");
		return mm;
	}
	mm.instantiate();

		...
		for (int m = 0; m < mesh_types.size(); m++) {
			...
			for (int c = 0; c < cell_locations.size(); c++) {
				...
				for (int lod = ma->get_last_lod(); lod >= Terrain3DMeshAsset::SHADOW_LOD_ID; lod--) {
					...
					if (mat.is_valid()) {
						RS->instance_geometry_set_material_override(mmi, mat->get_rid());
					}
					mat = ma->get_material_overlay();
					if (mat.is_valid()) {
						RS->instance_geometry_set_material_overlay(mmi, mat->get_rid());
					}
...
void Terrain3DInstancer::_setup_mmi_lod_ranges(const RID &p_mmi, const Ref<Terrain3DMeshAsset> &p_ma, const int p_lod) {
	...
	if (margin > 0.f) {
		...
		RS->instance_geometry_set_visibility_range(p_mmi, lod_begin, lod_end, lod_begin < 0.001f ? 0.f : margin, lod_end < 0.001f ? 0.f : margin, RenderingServer::VISIBILITY_RANGE_FADE_SELF);
	} else {
		real_t lod_begin = p_ma->get_lod_range_begin(p_lod);
		real_t lod_end = p_ma->get_lod_range_end(p_lod) * 1.0005f;
		RS->instance_geometry_set_visibility_range(p_mmi, lod_begin, lod_end, 0, 0, RenderingServer::VISIBILITY_RANGE_FADE_DISABLED);
	}
}
...
void Terrain3DInstancer::_destroy_mmi_by_cell(const Vector2i &p_region_loc, const int p_mesh_id, const Vector2i p_cell) {
	...
	for (int lod = Terrain3DMeshAsset::SHADOW_LOD_ID; lod < Terrain3DMeshAsset::MAX_LOD_COUNT; lod++) {
		...
		RID mmi = cell_mmi_dict[p_cell];
		LOG(EXTREME, "Freeing ", RID(mmi), " and erasing mmi cell ", p_cell);
		RS->free_rid(mmi);
...
void Terrain3DInstancer::dump_mmis() {
	...
	for (auto &i : _mmi_nodes) {
		...
		for (auto &j : i.second) {
			...
			for (auto &k : j.second) {
				LOG(MESG, "cell_mmi_dict cell: ", k.first, ", mmi rid: ", RID(k.second));
			}
		}
	}
}

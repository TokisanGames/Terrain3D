 ...
 */
String Pool3DManager::_cache_key(const Ref<Material> &p_base, const StringName &p_profile) const {
 ...
 * water, and Pool3D warns about it in Phase 3 -- but it must not error here every
 * frame.
 */
void Pool3DManager::_upload_into(const Ref<Material> &p_material, const Ref<Pasture3DWaveProfile> &p_profile) {
 ...
 */
void Pool3DManager::register_water_globals() {
...
void Pool3DManager::upload_profile_into(const Ref<Material> &p_material, const StringName &p_profile) {
	_upload_into(p_material, get_profile(p_profile));
}

void Pool3DManager::_on_profile_changed() {
	...
	// Bodies outside the cache (Ocean3D holds a private material, §6) have no other
	// way to learn a knob moved.
	emit_signal("profiles_changed");
}

void Pool3DManager::_connect_profiles() {
	Callable cb = callable_mp(this, &Pool3DManager::_on_profile_changed);
 ...
 */
void Pool3DManager::_update_clock(const double p_delta) {
	if (!_paused) {
		_water_time += p_delta;
	}
...
void Pool3DManager::_update_sun() {
...
void Pool3DManager::set_profiles(const TypedArray<Pasture3DWaveProfile> &p_profiles) {
...
void Pool3DManager::set_loop_period(const real_t p_seconds) {
...
void Pool3DManager::set_sun_light(Node3D *p_light) {
...
void Pool3DManager::set_paused(const bool p_paused) {
	_paused = p_paused;
}

Ref<Pasture3DWaveProfile> Pool3DManager::get_profile(const StringName &p_name) const {
	for (int i = 0; i < _profiles.size(); i++) {
		Ref<Pasture3DWaveProfile> profile = _profiles[i];
...
PackedStringArray Pool3DManager::get_profile_names() const {
...
Ref<Material> Pool3DManager::get_material_for(const Ref<Material> &p_base, const StringName &p_profile) {
	if (p_base.is_null()) {
		return Ref<Material>();
	}
...
void Pool3DManager::clear_material_cache() {
	_material_cache.clear();
}

void Pool3DManager::rebuild_tables() {
	for (int i = 0; i < _profiles.size(); i++) {
		Ref<Pasture3DWaveProfile> profile = _profiles[i];
		if (profile.is_valid()) {
			profile->set_loop_period(_loop_period);
...
real_t Pool3DManager::evaluate_height(const StringName &p_profile, const Vector2 &p_domain_xz) const {
	Ref<Pasture3DWaveProfile> profile = get_profile(p_profile);
	if (profile.is_null()) {
		return 0.f;
	}
...
Vector3 Pool3DManager::evaluate_normal(const StringName &p_profile, const Vector2 &p_domain_xz) const {
	Ref<Pasture3DWaveProfile> profile = get_profile(p_profile);
	if (profile.is_null()) {
...
Vector3 Pool3DManager::evaluate_surface_point(const StringName &p_profile, const Vector2 &p_domain_xz) const {
	Ref<Pasture3DWaveProfile> profile = get_profile(p_profile);
	if (profile.is_null()) {
...
Vector2 Pool3DManager::solve_domain(const StringName &p_profile, const Vector2 &p_target_xz) const {
	Ref<Pasture3DWaveProfile> profile = get_profile(p_profile);
	if (profile.is_null()) {
		return p_target_xz;
	}
...
PackedStringArray Pool3DManager::_get_configuration_warnings() const {
	PackedStringArray warnings;
	if (_active_managers > 1) {
		warnings.push_back(
				"There is more than one Pool3DManager in the tree. water_time is a project-wide "
...
void Pool3DManager::_notification(int p_what) {
	switch (p_what) {
		case NOTIFICATION_ENTER_TREE: {
			LOG(INFO, "NOTIFICATION_ENTER_TREE");
			_active_managers++;
			register_water_globals();
			if (_first_manager == nullptr) {
				_first_manager = this;
			}
		...
		case NOTIFICATION_EXIT_TREE: {
			...
			_active_managers = MAX(0, _active_managers - 1);
			if (_first_manager == this) {
				_first_manager = nullptr;
			}
...
void Pool3DManager::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_profiles", "profiles"), &Pool3DManager::set_profiles);
	ClassDB::bind_method(D_METHOD("get_profiles"), &Pool3DManager::get_profiles);
	ClassDB::bind_method(D_METHOD("set_loop_period", "seconds"), &Pool3DManager::set_loop_period);
	ClassDB::bind_method(D_METHOD("get_loop_period"), &Pool3DManager::get_loop_period);
	ClassDB::bind_method(D_METHOD("set_sun_light", "light"), &Pool3DManager::set_sun_light);
	ClassDB::bind_method(D_METHOD("get_sun_light"), &Pool3DManager::get_sun_light);
	ClassDB::bind_method(D_METHOD("set_paused", "paused"), &Pool3DManager::set_paused);
	ClassDB::bind_method(D_METHOD("is_paused"), &Pool3DManager::is_paused);

	ClassDB::bind_method(D_METHOD("get_water_time"), &Pool3DManager::get_water_time);
	ClassDB::bind_method(D_METHOD("get_profile", "name"), &Pool3DManager::get_profile);
	ClassDB::bind_method(D_METHOD("get_profile_names"), &Pool3DManager::get_profile_names);
	ClassDB::bind_method(D_METHOD("has_profile", "name"), &Pool3DManager::has_profile);

	ClassDB::bind_method(D_METHOD("get_material_for", "base", "profile"), &Pool3DManager::get_material_for);
	ClassDB::bind_method(D_METHOD("get_cached_material_count"), &Pool3DManager::get_cached_material_count);
	ClassDB::bind_method(D_METHOD("get_upload_count"), &Pool3DManager::get_upload_count);
	ClassDB::bind_method(D_METHOD("clear_material_cache"), &Pool3DManager::clear_material_cache);
	ClassDB::bind_method(D_METHOD("rebuild_tables"), &Pool3DManager::rebuild_tables);
	ClassDB::bind_method(D_METHOD("upload_profile_into", "material", "profile"), &Pool3DManager::upload_profile_into);

	ADD_SIGNAL(MethodInfo("profiles_changed"));

	ClassDB::bind_method(D_METHOD("evaluate_height", "profile", "domain_xz"), &Pool3DManager::evaluate_height);
	ClassDB::bind_method(D_METHOD("evaluate_normal", "profile", "domain_xz"), &Pool3DManager::evaluate_normal);
	ClassDB::bind_method(D_METHOD("evaluate_surface_point", "profile", "domain_xz"), &Pool3DManager::evaluate_surface_point);
	ClassDB::bind_method(D_METHOD("solve_domain", "profile", "target_xz"), &Pool3DManager::solve_domain);

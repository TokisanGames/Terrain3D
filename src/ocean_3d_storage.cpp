...
void Ocean3DStorage::set_version(real_t p_version) {
...
void Ocean3DStorage::set_save_16_bit(bool p_enabled) {
	LOG(INFO, p_enabled);
	_save_16_bit = p_enabled;
}

void Ocean3DStorage::set_height_range(Vector2 p_range) {
	LOG(INFO, vformat("%.2v", p_range));
	_height_range = p_range;
}

void Ocean3DStorage::update_heights(real_t p_height) {
...
void Ocean3DStorage::update_heights(Vector2 p_heights) {
...
void Ocean3DStorage::update_height_range() {
...
void Ocean3DStorage::clear_edited_area() {
	_edited_area = AABB();
}

void Ocean3DStorage::add_edited_area(AABB p_area) {
...
AABB Ocean3DStorage::get_edited_area() const {
...
void Ocean3DStorage::set_region_size(RegionSize p_size) {
	LOG(INFO, p_size);
...
void Ocean3DStorage::set_region_offsets(const TypedArray<Vector2i> &p_offsets) {
...
Vector2i Ocean3DStorage::get_region_offset(Vector3 p_global_position) {
...
int Ocean3DStorage::get_region_index(Vector3 p_global_position) {
 ...
 */
Error Ocean3DStorage::add_region(Vector3 p_global_position, const TypedArray<Image> &p_images, bool p_update) {
	Vector2i uv_offset = get_region_offset(p_global_position);
...
void Ocean3DStorage::remove_region(Vector3 p_global_position, bool p_update) {
...
void Ocean3DStorage::update_regions(bool force_emit) {
...
void Ocean3DStorage::set_map_region(MapType p_map_type, int p_region_index, const Ref<Image> p_image) {
	switch (p_map_type) {
		case TYPE_HEIGHT:
...
Ref<Image> Ocean3DStorage::get_map_region(MapType p_map_type, int p_region_index) const {
	switch (p_map_type) {
		case TYPE_HEIGHT:
			if (p_region_index >= 0 && p_region_index < _height_maps.size()) {
...
void Ocean3DStorage::set_maps(MapType p_map_type, const TypedArray<Image> &p_maps) {
...
TypedArray<Image> Ocean3DStorage::get_maps(MapType p_map_type) const {
...
TypedArray<Image> Ocean3DStorage::get_maps_copy(MapType p_map_type) const {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
		LOG(ERROR, "Specified map type out of range");
		return TypedArray<Image>();
	}
...
void Ocean3DStorage::set_pixel(MapType p_map_type, Vector3 p_global_position, Color p_pixel) {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
		LOG(ERROR, "Specified map type out of range");
		return;
	}
...
Color Ocean3DStorage::get_pixel(MapType p_map_type, Vector3 p_global_position) {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
		LOG(ERROR, "Specified map type out of range");
		return COLOR_NAN;
	}
 ...
 **/
Vector3 Ocean3DStorage::get_texture_id(Vector3 p_global_position) {
 ...
 **/
TypedArray<Image> Ocean3DStorage::sanitize_maps(MapType p_map_type, const TypedArray<Image> &p_maps) {
...
void Ocean3DStorage::force_update_maps(MapType p_map_type) {
	switch (p_map_type) {
		case TYPE_HEIGHT:
			_generated_height_maps.clear();
			break;
...
void Ocean3DStorage::save() {
	...
	if (path.get_extension() == "tres" || path.get_extension() == "res") {
		...
		if (_save_16_bit) {
			...
			original_maps = get_maps_copy(Ocean3DStorage::MapType::TYPE_HEIGHT);
			for (int i = 0; i < _height_maps.size(); i++) {
 ...
 */
Ref<Image> Ocean3DStorage::load_image(String p_file_name, int p_cache_mode, Vector2 p_r16_height_range, Vector2i p_r16_size) {
...
/**
 * Imports an Image set (Height, Control, Color) into Ocean3DStorage
 ...
 */
void Ocean3DStorage::import_images(const TypedArray<Image> &p_images, Vector3 p_global_position, real_t p_offset, real_t p_scale) {
 ...
 */
Error Ocean3DStorage::export_image(String p_file_name, MapType p_map_type) {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
...
Ref<Image> Ocean3DStorage::layered_to_image(MapType p_map_type) {
 ...
 */
Vector3 Ocean3DStorage::get_mesh_vertex(int32_t p_lod, HeightFilter p_filter, Vector3 p_global_position) {
...
Vector3 Ocean3DStorage::get_normal(Vector3 p_global_position) {
	int region = get_region_index(p_global_position);
...
void Ocean3DStorage::print_audit_data() {
...
void Ocean3DStorage::_bind_methods() {
	...
	BIND_ENUM_CONSTANT(SIZE_8192);
	BIND_ENUM_CONSTANT(SIZE_16384);
	BIND_ENUM_CONSTANT(SIZE_32768);
	...
	ClassDB::bind_method(D_METHOD("set_version", "version"), &Ocean3DStorage::set_version);
	ClassDB::bind_method(D_METHOD("get_version"), &Ocean3DStorage::get_version);
	ClassDB::bind_method(D_METHOD("set_save_16_bit", "enabled"), &Ocean3DStorage::set_save_16_bit);
	ClassDB::bind_method(D_METHOD("get_save_16_bit"), &Ocean3DStorage::get_save_16_bit);

	ClassDB::bind_method(D_METHOD("set_height_range", "range"), &Ocean3DStorage::set_height_range);
	ClassDB::bind_method(D_METHOD("get_height_range"), &Ocean3DStorage::get_height_range);
	ClassDB::bind_method(D_METHOD("update_height_range"), &Ocean3DStorage::update_height_range);

	ClassDB::bind_method(D_METHOD("set_region_size", "size"), &Ocean3DStorage::set_region_size);
	ClassDB::bind_method(D_METHOD("get_region_size"), &Ocean3DStorage::get_region_size);
	ClassDB::bind_method(D_METHOD("set_region_offsets", "offsets"), &Ocean3DStorage::set_region_offsets);
	ClassDB::bind_method(D_METHOD("get_region_offsets"), &Ocean3DStorage::get_region_offsets);
	ClassDB::bind_method(D_METHOD("get_region_count"), &Ocean3DStorage::get_region_count);
	ClassDB::bind_method(D_METHOD("get_region_offset", "global_position"), &Ocean3DStorage::get_region_offset);
	ClassDB::bind_method(D_METHOD("get_region_index", "global_position"), &Ocean3DStorage::get_region_index);
	ClassDB::bind_method(D_METHOD("has_region", "global_position"), &Ocean3DStorage::has_region);
	ClassDB::bind_method(D_METHOD("add_region", "global_position", "images", "update"), &Ocean3DStorage::add_region, DEFVAL(TypedArray<Image>()), DEFVAL(true));
	ClassDB::bind_method(D_METHOD("remove_region", "global_position", "update"), &Ocean3DStorage::remove_region, DEFVAL(true));

	ClassDB::bind_method(D_METHOD("set_map_region", "map_type", "region_index", "image"), &Ocean3DStorage::set_map_region);
	ClassDB::bind_method(D_METHOD("get_map_region", "map_type", "region_index"), &Ocean3DStorage::get_map_region);
	ClassDB::bind_method(D_METHOD("set_maps", "map_type", "maps"), &Ocean3DStorage::set_maps);
	ClassDB::bind_method(D_METHOD("get_maps", "map_type"), &Ocean3DStorage::get_maps);
	ClassDB::bind_method(D_METHOD("get_maps_copy", "map_type"), &Ocean3DStorage::get_maps_copy);
	ClassDB::bind_method(D_METHOD("set_height_maps", "maps"), &Ocean3DStorage::set_height_maps);
	ClassDB::bind_method(D_METHOD("get_height_maps"), &Ocean3DStorage::get_height_maps);
	ClassDB::bind_method(D_METHOD("set_control_maps", "maps"), &Ocean3DStorage::set_control_maps);
	ClassDB::bind_method(D_METHOD("get_control_maps"), &Ocean3DStorage::get_control_maps);
	ClassDB::bind_method(D_METHOD("set_color_maps", "maps"), &Ocean3DStorage::set_color_maps);
	ClassDB::bind_method(D_METHOD("get_color_maps"), &Ocean3DStorage::get_color_maps);
	ClassDB::bind_method(D_METHOD("set_pixel", "map_type", "global_position", "pixel"), &Ocean3DStorage::set_pixel);
	ClassDB::bind_method(D_METHOD("get_pixel", "map_type", "global_position"), &Ocean3DStorage::get_pixel);
	ClassDB::bind_method(D_METHOD("set_height", "global_position", "height"), &Ocean3DStorage::set_height);
	ClassDB::bind_method(D_METHOD("get_height", "global_position"), &Ocean3DStorage::get_height);
	ClassDB::bind_method(D_METHOD("set_color", "global_position", "color"), &Ocean3DStorage::set_color);
	ClassDB::bind_method(D_METHOD("get_color", "global_position"), &Ocean3DStorage::get_color);
	ClassDB::bind_method(D_METHOD("set_control", "global_position", "control"), &Ocean3DStorage::set_control);
	ClassDB::bind_method(D_METHOD("get_control", "global_position"), &Ocean3DStorage::get_control);
	ClassDB::bind_method(D_METHOD("set_roughness", "global_position", "roughness"), &Ocean3DStorage::set_roughness);
	ClassDB::bind_method(D_METHOD("get_roughness", "global_position"), &Ocean3DStorage::get_roughness);
	ClassDB::bind_method(D_METHOD("get_texture_id", "global_position"), &Ocean3DStorage::get_texture_id);
	ClassDB::bind_method(D_METHOD("force_update_maps", "map_type"), &Ocean3DStorage::force_update_maps, DEFVAL(TYPE_MAX));

	ClassDB::bind_method(D_METHOD("save"), &Ocean3DStorage::save);
	ClassDB::bind_static_method("Ocean3DStorage", D_METHOD("load_image", "file_name", "cache_mode", "r16_height_range", "r16_size"), &Ocean3DStorage::load_image, DEFVAL(ResourceLoader::CACHE_MODE_IGNORE), DEFVAL(Vector2(0, 255)), DEFVAL(Vector2i(0, 0)));
	ClassDB::bind_method(D_METHOD("import_images", "images", "global_position", "offset", "scale"), &Ocean3DStorage::import_images, DEFVAL(Vector3(0, 0, 0)), DEFVAL(0.0), DEFVAL(1.0));
	ClassDB::bind_method(D_METHOD("export_image", "file_name", "map_type"), &Ocean3DStorage::export_image);
	ClassDB::bind_method(D_METHOD("layered_to_image", "map_type"), &Ocean3DStorage::layered_to_image);

	ClassDB::bind_method(D_METHOD("get_mesh_vertex", "lod", "filter", "global_position"), &Ocean3DStorage::get_mesh_vertex);
	ClassDB::bind_method(D_METHOD("get_normal", "global_position"), &Ocean3DStorage::get_normal);
	...
	ADD_PROPERTY(PropertyInfo(Variant::INT, "region_size", PROPERTY_HINT_ENUM, "1024:1024, 2048:2048, 4096:4096, 8192:8192, 16384:16384, 32768:32768"), "set_region_size", "get_region_size");

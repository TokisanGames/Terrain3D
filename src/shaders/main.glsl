...
//INSERT: DUAL_SCALING
	...
	//INSERT: DUAL_SCALING_CONDITION_0
		...
		vec4 id_dd = (projected ? p_dd : i_dd) * id_scale;

		// Detiling and Control map rotation
		vec2 uv_center = floor(fma((projected ? p_pos : i_pos), vec2(id_scale), vec2(0.5)));
		...
		vec2 id_uv = rotate_vec2(fma((projected ? p_uv : i_uv), vec2(id_scale), -uv_center), id_cs_angle) + uv_center + id_detile.y - 0.5;
		...
		nrm.xz = rotate_vec2(nrm.xz, id_cs_angle) * (projected ? p_align : mat2(1.));

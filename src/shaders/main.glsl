...
void get_material(vec3 i_normal, float i_height, vec4 ddxy, uint control, ivec3 iuv_center, mat3 TANGENT_WORLD_MATRIX, out Material out_mat) {
	...
	if (region < 0 || i_normal.y >= projection_threshold || !enable_projection) {
	    ...
	    p_angle = atan(-TANGENT_WORLD_MATRIX[1].x, -TANGENT_WORLD_MATRIX[1].z);
...
//INSERT: DUAL_SCALING_OVERLAY
		...
		// apply world space normal weighting from base, or overlay layer
		// TODO
		// Add control map bit to select the correct weight to adjust - default is base layer.
		// Maybe a better way to handle this? Has to work with height blending and currrent painted blend weight.
		// Should also consider potential displacement effects that rely on the height height value.
		albedo_ht.a *= bool(control >>3u & 0x1u) ? clamp((TANGENT_WORLD_MATRIX * normal_rg2.xyz).y, 0.0, 1.0) : 1.0;

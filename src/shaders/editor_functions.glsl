...
vec3 get_decal(vec3 albedo, vec2 uv) {
	...
	// Visual debug: show which _editor_decal_visible[i] are true
	for (int j = 0; j < 3; ++j) {
		vec2 marker_pos = vec2(-1. + 1. * float(j), -1.); // Spread out markers near origin
		float marker_radius = 2.;
		float marker_dist = length((uv * _vertex_spacing) - marker_pos);
		float marker = smoothstep(marker_radius, marker_radius - 0.01, marker_dist);
		vec3 marker_color = vec3(float(j == 0), float(j == 1), float(j == 2)); // Red, Green, Blue
		float intensity = _editor_decal_visible[j] ? 1.0 : 0.02;
		albedo = mix(albedo, marker_color, marker * intensity);
	}

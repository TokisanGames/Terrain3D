...
void accumulate_material(const mat3 TNB, const float weight, const ivec3 index,
	...
	if (blend < 1.0) {
		...
		float height_scale = 1.0 / (control_scale * id_scale) * _texture_displacement_array[id].y * 0.04;
		h = clamp(fma(h, height_scale, fma(-0.5, height_scale, _texture_displacement_array[id].x + 0.5)), 0., 1.);

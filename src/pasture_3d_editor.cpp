...
void Pasture3DEditor::_send_region_aabb(const Vector2i &p_region_loc, const Vector2 &p_height_range) {
	Pasture3D::RegionSize region_size = _terrain->get_region_size();
	AABB edited_area;
...
Ref<Pasture3DRegion> Pasture3DEditor::_operate_region(const Vector2i &p_region_loc) {
	bool changed = false;
	...
	Ref<Pasture3DRegion> region = data->get_region(p_region_loc);
	if (can_print) {
...
void Pasture3DEditor::_operate_map(const Vector3 &p_global_position, const real_t p_camera_direction) {
	...
	for (real_t x = 0.f; x < brush_size; x += vertex_spacing) {
		for (real_t y = 0.f; y < brush_size; y += vertex_spacing) {
			...
			Ref<Pasture3DRegion> region = _operate_region(region_loc);
	...
	if (map_type == TYPE_COLOR) {
		for (Ref<Pasture3DRegion> region : _edited_regions) {
			if (region.is_valid()) {
	...
	if (_terrain->get_collision_mode() == Pasture3DCollision::DYNAMIC_EDITOR) {
...
void Pasture3DEditor::_notify_layer_blocked(const Ref<Pasture3DLayer> &p_layer, BlockReason p_reason) const {
	...
	if (plugin && plugin->has_method("flash_layer_warning")) {
		plugin->call("flash_layer_warning", p_layer->get_layer_name(), hidden);
	}
}

void Pasture3DEditor::_store_undo() {
	...
	if (Pasture3D::debug_level >= DEBUG) {
		...
		for (const Ref<Pasture3DRegion> &region : _edited_regions) {
			if (region.is_valid()) {
				region->dump();
			}
		}
	...
	if (_stroke_layer.is_valid() && !_layer_undo_tiles.is_empty()) {
		// Deep-clone the tile snapshots so each committed action owns its own Image data. Storing the
		// members directly would alias them, and stop_operation()'s clear() below would then leave the
	...
	if (_terrain->get_plugin()->has_method("create_undo_action")) {
		...
		String action_name = String("Pasture3D ") + OPNAME[_operation] + String(" ") + TOOLNAME[_tool];
		...
		_terrain->get_plugin()->call("add_do_method", Callable(this, "apply_undo").bind(redo_data.duplicate()));
...
void Pasture3DEditor::_apply_undo(const Dictionary &p_data) {
	IS_INIT_COND_MESG(!_terrain->get_plugin(), "_terrain isn't initialized, returning", VOID);
	...
	if (p_data.has("edited_regions")) {
		...
		TypedArray<Pasture3DRegion> undo_regions = p_data["edited_regions"];
...
float Pasture3DEditor::_average(const AverageMode p_mode, const Vector3 &p_global_position, const float p_base,
...
Color Pasture3DEditor::_average(const Vector3 &p_global_position, const Color &p_base) const {
...
void Pasture3DEditor::set_brush_data(const Dictionary &p_data) {
	...
	_brush_data["asset_id"] = CLAMP(int(p_data.get("asset_id", 0)), 0, ((_tool == INSTANCER) ? Pasture3DAssets::MAX_MESHES : Pasture3DAssets::MAX_TEXTURES) - 1);
...
void Pasture3DEditor::set_tool(const Tool p_tool) {
	Tool old_tool = _tool;
	...
	if (_terrain && (_tool == Tool::NAVIGATION || old_tool == Tool::NAVIGATION || _tool == Tool::REGION || old_tool == Tool::REGION)) {
		_terrain->get_material()->update(Pasture3DMaterial::FULL_REBUILD);
	}
}

void Pasture3DEditor::set_operation(const Operation p_operation) {
...
void Pasture3DEditor::operate(const Vector3 &p_global_position, const real_t p_camera_direction) {
	IS_DATA_INIT_MESG("Terrain isn't initialized", VOID);
	if (!_is_operating) {
...
void Pasture3DEditor::backup_region(const Ref<Pasture3DRegion> &p_region) {
	...
	if (_is_operating && p_region.is_valid() && !p_region->is_edited()) {
		...
		Ref<Pasture3DRegion> orig_region = p_region->duplicate(true);
...
void Pasture3DEditor::stop_operation() {
	IS_DATA_INIT_MESG("Terrain isn't initialized", VOID);
	...
	if (_is_operating && (!_added_removed_locations.is_empty() || !_edited_regions.is_empty())) {
		for (int i = 0; i < _edited_regions.size(); i++) {
			Ref<Pasture3DRegion> region = _edited_regions[i];
			region->set_edited(false);
			// Make duplicate for redo, necessary or redos won't work
			Ref<Pasture3DRegion> redo_region = region->duplicate(true);
...
void Pasture3DEditor::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_tool", "tool"), &Pasture3DEditor::set_tool);
	ClassDB::bind_method(D_METHOD("get_tool"), &Pasture3DEditor::get_tool);
	ClassDB::bind_method(D_METHOD("set_operation", "operation"), &Pasture3DEditor::set_operation);
	ClassDB::bind_method(D_METHOD("get_operation"), &Pasture3DEditor::get_operation);
	ClassDB::bind_method(D_METHOD("start_operation", "position"), &Pasture3DEditor::start_operation);
	ClassDB::bind_method(D_METHOD("is_operating"), &Pasture3DEditor::is_operating);
	ClassDB::bind_method(D_METHOD("operate", "position", "camera_direction"), &Pasture3DEditor::operate);
	ClassDB::bind_method(D_METHOD("backup_region", "region"), &Pasture3DEditor::backup_region);
	ClassDB::bind_method(D_METHOD("stop_operation"), &Pasture3DEditor::stop_operation);

	ClassDB::bind_method(D_METHOD("apply_undo", "data"), &Pasture3DEditor::_apply_undo);
}

#ifndef OCEAN3D_STORAGE_CLASS_H
#define OCEAN3D_STORAGE_CLASS_H
...
class Ocean3DStorage : public Resource {
	GDCLASS(Ocean3DStorage, Resource);

public:
	// Constants
	static inline const char *__class__ = "Ocean3DStorage";
...
VARIANT_ENUM_CAST(Ocean3DStorage::MapType);
VARIANT_ENUM_CAST(Ocean3DStorage::RegionSize);
VARIANT_ENUM_CAST(Ocean3DStorage::HeightFilter);

// Inline Functions

inline void Ocean3DStorage::set_height(Vector3 p_global_position, real_t p_height) {
	set_pixel(TYPE_HEIGHT, p_global_position, Color(p_height, 0.f, 0.f, 1.f));
}

inline real_t Ocean3DStorage::get_height(Vector3 p_global_position) {
...
inline void Ocean3DStorage::set_color(Vector3 p_global_position, Color p_color) {
	p_color.a = get_roughness(p_global_position);
	set_pixel(TYPE_COLOR, p_global_position, p_color);
}

inline Color Ocean3DStorage::get_color(Vector3 p_global_position) {
	Color clr = get_pixel(TYPE_COLOR, p_global_position);
	clr.a = 1.0f;
	return clr;
}

inline void Ocean3DStorage::set_control(Vector3 p_global_position, uint32_t p_control) {
	set_pixel(TYPE_CONTROL, p_global_position, Color(as_float(p_control), 0.f, 0.f, 1.f));
}

inline uint32_t Ocean3DStorage::get_control(Vector3 p_global_position) {
	return as_uint(get_pixel(TYPE_CONTROL, p_global_position).r);
}

inline void Ocean3DStorage::set_roughness(Vector3 p_global_position, real_t p_roughness) {
	Color clr = get_pixel(TYPE_COLOR, p_global_position);
	clr.a = p_roughness;
	set_pixel(TYPE_COLOR, p_global_position, clr);
}

inline real_t Ocean3DStorage::get_roughness(Vector3 p_global_position) {
	return get_pixel(TYPE_COLOR, p_global_position).a;
}

#endif // OCEAN3D_STORAGE_CLASS_H

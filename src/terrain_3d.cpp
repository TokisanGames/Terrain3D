...
void Terrain3D::_initialize() {
	...
	if (!_assets->is_connected("meshes_changed", callable_mp(_instancer, &Terrain3DInstancer::update_mmis).bindv({ false, V2I_MAX, -1 }))) {
		LOG(DEBUG, "Connecting _assets.meshes_changed to _instancer->update_mmis()");
		_assets->connect("meshes_changed", callable_mp(_instancer, &Terrain3DInstancer::update_mmis).bindv({ false, V2I_MAX, -1 }));
	}

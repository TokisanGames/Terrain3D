...
**Status:** **BUILT except P8** (verified against the codebase 2026-09-02). P0-P2, P4a/P4b and
P5-P7b are shipped with gates; there is no P3 (see §11). **P8 (auto-routing) is the only phase with
no code** - no anisotropic A*, no `RoadRoutingGate`. The original header read "PROPOSAL, nothing
built" long after P0 landed; do not plan from a status line without checking the symbols.

v1 written 2026-08-30 from a review of the terrain node graph and a
...
| **P2** ✅ | `Pasture3DRoadModifier` in the brush stack — grading + masks. GDScript first, then native, then GPU, on the existing three-tier discipline. **The native tier is specified in `PASTURE3D_GRAPH_GEOMETRY_PORTS_SPEC.md`** (2026-08-31), which also corrects this row: there is no `BrushModStep` enum in `src/`, so the work is a `Pasture3DUtil` kernel plus a geometry operand in the graph program, not a case in a dispatcher. | `RoadModifierGate` — graded height vs an independent re-derivation; mask algebra; freeze/stale/Bake; NaN passthrough at the brush-loop boundary. Plus a native A/B and `RoadGpuParityGate`. |

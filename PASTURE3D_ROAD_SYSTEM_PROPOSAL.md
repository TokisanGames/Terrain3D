...
**Status 2026-08-31: the lane graph and right of way are built; the sufficiency check is not.** `RoadLaneGraphGate` runs fourteen criteria (A–N) covering the cross-section, connectors, stop lines, conflicts, yield rules and the signal cycle. Two decisions are worth recording because both were arrived at by first writing the other one:
